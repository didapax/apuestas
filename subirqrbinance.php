  ...
  if (isset($archivo) && $archivo != "") {
    ...
    if (!((strpos($tipo, "jpeg") || strpos($tipo, "jpg") || strpos($tipo, "webp") || strpos($tipo, "png")) && ($tamano < 500000))) {
    	die ("<br><br>Error. La extensión o el tamaño de los archivos no es correcta.<br>Actualice la Pagina y Intente de Nuevo");
			...
			$imagen="Assets/Perfiles/".readClienteId($_SESSION['user'])['QR_BINANCE'];
			if($imagen != "Assets/Perfiles/perfil.jpg") {
				if(file_exists ($imagen))unlink($imagen);
			}
      $fileImagen='Assets/Perfiles/'.$codigorifa.$ext;
      if (move_uploaded_file($temp,$fileImagen)) {
            chmod($fileImagen, 0777);
			sqlconector("UPDATE USUARIOS SET QR_BINANCE='".$codigorifa.$ext."' WHERE ID=".$_SESSION['user']);						
      }
   	else {
...
<body>
<form id="target" action="subirqrbinance" method="POST" enctype="multipart/form-data" style='text-align: center;'>
<label for="archivo" >
	<img class='img-perfil' style="border: 1px solid black; border-radius:5px;"  width='50' height='50' src='Assets/Perfiles/<?php echo readClienteId($_SESSION['user'])['QR_BINANCE']?>' alt=''><br>
</label>
	<input style="display: none;" accept='image/*' type='file' id='archivo' name='archivo' accept=".jpg, .jpeg, .png, .webp" onchange="hola()">
	<label class="upload-btn" style="cursor:pointer;background:gray;border: solid 1px black; border-radius:3px;color:white;" for="archivo" >Subir QR</label>
