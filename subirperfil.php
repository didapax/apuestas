  ...
  if (isset($archivo) && $archivo != "") {
    ...
    if (!((strpos($tipo, "jpeg") || strpos($tipo, "jpg") || strpos($tipo, "webp") || strpos($tipo, "png")) && ($tamano < 500000))) {
    	die ("<br><br>Error. La extensión o el tamaño de los archivos no es correcta.<br>Actualice la Pagina y Intente de Nuevo");
			...
			$imagen="Assets/Perfiles/".readClienteId($_SESSION['user'])['PERFIL'];
      ...
      if (move_uploaded_file($temp,$fileImagen)) {
			...
			sqlconector("UPDATE USUARIOS SET PERFIL='".$codigorifa.$ext."' WHERE ID=".$_SESSION['user']);						
      }
   	else {
...
<label for="archivo" >
	<img class='img-perfil' style="border: 2px double black; padding:3px; border-radius:5px;"  width='150' height='150' src='Assets/Perfiles/<?php echo readClienteId($_SESSION['user'])['PERFIL']?>' alt=''><br>
</label>
	<input style="display: none;" accept='image/*' type='file' id='archivo' name='archivo' accept=".jpg, .jpeg, .png, .webp" onchange="hola()">
	<label class="upload-btn" style="cursor:pointer;background:gray;border: solid 1px black; border-radius:3px;color:white;" for="archivo" >Subir Foto Perfil</label>
