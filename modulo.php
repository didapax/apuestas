...
$GLOBALS['__RETIROSUSDC__'] = row_sqlconector("SELECT SUM(RECIBE) AS TOTAL FROM TRANSACCIONES WHERE TIPO='DEPOSITO'")['TOTAL'];
$GLOBALS['__RETIROS__'] = row_sqlconector("SELECT COUNT(*) AS TOTAL FROM TRANSACCIONES WHERE TIPO='RETIRO'")['TOTAL'];
$GLOBALS['__RETIROSUSDC__'] = row_sqlconector("SELECT SUM(RECIBE) AS TOTAL FROM TRANSACCIONES WHERE TIPO='RETIRO'")['TOTAL'];
$GLOBALS['__TOTALSALDOS__'] = row_sqlconector("SELECT SUM(SALDO) AS TOTAL FROM USUARIOS")['TOTAL'];
$GLOBALS['__PAGOSMESACTUAL__'] = row_sqlconector("SELECT SUM(INTERES_MENSUAL) AS TOTAL FROM LIBROCONTABLE WHERE TIPO = 'CREDITO' AND MONTH(FECHA) = MONTH(CURDATE()) AND YEAR(FECHA) = YEAR(CURDATE())")['TOTAL'];
